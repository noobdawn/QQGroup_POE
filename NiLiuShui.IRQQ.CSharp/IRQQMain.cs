...
namespace NiLiuShui.IRQQ.CSharp
{
    ...
    public class IRQQMain
    {
        ...
        {
            string szBuffer = "插件名称{NiLiuShui}\n插件版本{1.0.0}\n插件作者{情事}\n插件说明{这是一个C#语言的IRQQ插件SDK模板，请自行修改使用}\n插件skey{8956RTEWDFG3216598WERDF3}插件sdk{S3}";
            Timeslice.Open();
            DataRunTime.Load();
            modules = new IModule[]
            {
        ...
        {
            IRQQApi.Api_OutPutLog(string.Format("MsgType:{0}, MsgCType:{1}, MsgFrom:{2}, TigObjF:{3}, TigObjC:{4}", MsgType, MsgCType, MsgFrom, TigObjF, TigObjC));
            if(MsgType == 1)
            {
