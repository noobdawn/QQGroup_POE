    ...
    class Arena : IModule
    {
        public bool WhenParamIn(SendParam param)
        {
            ...
            if (param.param[0] == _S.GetText("Command_to_ChaosStrike") && param.target != null)
            {
                ...
                var realDmg = _S.Damage(param.caster, param.target, dmg, true, true);
                _S.Response(param.GroupQQ, param.QQ, "Response_to_ChaosStrike", param.caster.QQ, param.target.QQ, cost, -realDmg);
                return false;
            }
